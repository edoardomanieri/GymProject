...
-- * Generation date: Wed May 09 10:09:31 2018 
...
create table PIANOALLENAMENTO (
     ...
     constraint FKconfigura_ID primary key (ID));
...
alter table ALLENAMENTI add constraint FKrealizza_FK
     foreign key (Rea_ID)
     references UTENTE;
...
alter table PIANOALLENAMENTO add constraint FKconfigura_CHK
     check(exists(select * from GIORNIALLENAMENTI
                  where GIORNIALLENAMENTI.Con_ID = ID)); 

alter table PIANOALLENAMENTO add constraint FKconfigura_FK
     foreign key (ID)
     references UTENTE;
...
create index FKrealizza_IND
     on ALLENAMENTI (Rea_ID);
...
create unique index FKconfigura_IND
